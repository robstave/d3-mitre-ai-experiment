...
<div id="chart-type-selector">
    <label>
        <input type="radio" name="chartType" value="radar" checked onchange="toggleChart(this.value)"> Radar Chart
    </label>
    <label>
        <input type="radio" name="chartType" value="bar" onchange="toggleChart(this.value)"> Bar Chart
    </label>
</div>
```
...
<head>
    ...
    <style>
        ...
        #chart { width: 100%; height: 600px; background-color: white; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); }
        .tooltip {
            position: absolute;
        ...
        #timeframe-selector { margin-bottom: 20px; }
...
<body>
    <div id="chart-container">
        ...
        <p>This chart displays the cumulative prevalence of different tactics in cybersecurity attacks over time.</p>
        <div id="timeframe-selector">
            <button onclick="updateTimeframe('24h')">24 Hours</button>
            <button onclick="updateTimeframe('1w')">1 Week</button>
            <button onclick="updateTimeframe('1m')">1 Month</button>
